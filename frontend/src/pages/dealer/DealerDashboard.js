...
function DealerDashboard() {
  ...
  const loadCars = (sellerId) => {
    axios.get(`projectautohub-production-2c65.up.railway.app/api/cars/seller/${sellerId}`).then(res => {
      setCars(res.data);
  ...
  const loadTransactions = (sellerId) => {
    axios.get(`projectautohub-production-2c65.up.railway.app/api/cars/transactions/${sellerId}`).then(res => {
  ...
  const handleAddCar = async () => {
    ...
    await axios.post('projectautohub-production-2c65.up.railway.app/api/cars/add', data);
  ...
  const handleDelete = async (id) => {
    await axios.delete(`projectautohub-production-2c65.up.railway.app/api/cars/${id}`);
    loadCars(user.id);
  };

  const handleConfirmSale = async (id) => {
    await axios.post(`projectautohub-production-2c65.up.railway.app/api/cars/confirm/${id}`);
