...
function RentalDashboard() {
  ...
  const loadCars = (agencyId) => {
    axios.get(`${API_URL}/api/rentals/agency/${agencyId}`).then(res => {
      setCars(res.data);
  ...
  const loadTransactions = (agencyId) => {
    axios.get(`${API_URL}/api/rentals/transactions/${agencyId}`).then(res => {
  ...
  const handleAddCar = async () => {
    ...
    await axios.post('${API_URL}/api/rentals/add', data);
  ...
  const handleConfirmRental = async (id) => {
    await axios.post(`${API_URL}/api/rentals/confirm/${id}`);
  ...
  const handleMarkAvailable = async (id) => {
    await axios.post(`${API_URL}/api/rentals/return/${id}`);
    loadCars(user.id);
  };
