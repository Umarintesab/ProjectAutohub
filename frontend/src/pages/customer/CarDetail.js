...
function CarDetail() {
  ...
  const fetchCar = async () => {
    if (type === 'rental') {
      const res = await axios.get('$env:REACT_APP_API_URL/api/rentals');
      const found = res.data.find(c => c.id === carId);
      setCar(found);
      if (found?.transactionId) setTransactionId(found.transactionId);
    } else {
      const res = await axios.get('$env:REACT_APP_API_URL/api/cars');
      const found = res.data.find(c => c.id === carId);
      setCar(found);
      if (found?.transactionId) setTransactionId(found.transactionId);
    }
  ...
  const handleBuy = async () => {
    ...
    await axios.post(`$env:REACT_APP_API_URL/api/cars/buy/${car.id}`, {
      buyerId: user.id,
  ...
  const handleRent = async () => {
    const user = JSON.parse(localStorage.getItem('user'));
    await axios.post(`$env:REACT_APP_API_URL/api/rentals/rent/${car.id}`, {
      renterId: user.id,
  ...
  const checkTransaction = async () => {
    ...
    try {
      if (type === 'rental') {
        const res = await axios.get(`$env:REACT_APP_API_URL/api/rentals/check-transaction/${carId}`);
        if (res.data.transactionId) setTransactionId(res.data.transactionId);
      } else {
        const res = await axios.get(`$env:REACT_APP_API_URL/api/cars/check-transaction/${carId}`);
        if (res.data.transactionId) setTransactionId(res.data.transactionId);
      }
    } catch (e) {}
    setChecking(false);
